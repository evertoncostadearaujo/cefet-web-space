...
const caminho = 'http://fegemo.github.io/cefet-web/images/osiris/'
...
anteriorEl.addEventListener('click', function() {
    let arquivo = slideEl.src.replace(caminho, '')
    let indice = imagens.findIndex(imagem => imagem.arquivo === arquivo)
    if (indice == 0) {
        slideEl.src = caminho.concat(imagens[7].arquivo)
        slideEl.alt = imagens[7].descricao
    } else {
        indice--
        slideEl.src = caminho.concat(imagens[indice].arquivo)
        slideEl.alt = imagens[indice].descricao
    }
});
proximoEl.addEventListener('click', function() {
    ...
    let indice = imagens.findIndex(imagem => imagem.arquivo === arquivo)
    if (indice == 7) {
        slideEl.src = caminho.concat(imagens[0].arquivo)
        slideEl.alt = imagens[0].descricao
    } else {
        indice++
        slideEl.src = caminho.concat(imagens[indice].arquivo)
        slideEl.alt = imagens[indice].descricao
